...
fn binom_k_larger_equals(p: f64, n: f64, k: f64) -> f64 {
    let chance: f64 = (k as u64..n as u64+1_u64).collect::<Vec<u64>>().iter().map(|_k| binom(p, n, *_k as f64)).sum();
    chance
}
...
fn main() {
    ...
    println!("at least 80% success rate in 5 startups: {}", binom_k_larger_equals(STARTUP_SUCCESS_RATE, 5., 4.));
